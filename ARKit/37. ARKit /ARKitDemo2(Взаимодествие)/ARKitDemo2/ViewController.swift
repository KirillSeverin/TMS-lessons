...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // подписываемся под contactDelegate
...
extension ViewController {
    ...
    func createBox(hitResult: ARRaycastResult) {
        ...
        let position = SCNVector3(hitResult.worldTransform.columns.3.x,
                                  hitResult.worldTransform.columns.3.y + 0.3, // 0.3 - подняли для падения
...
//        sceneView.scene.rootNode.addChildNode(ship)
                ...
                let position2 = SCNVector3(hitResult.worldTransform.columns.3.x,
                                           hitResult.worldTransform.columns.3.y + 0.15,
                                           hitResult.worldTransform.columns.3.z + 0.5)
                let scene = SCNScene(named: "art.scnassets/Handgun_dae copy.scn")!
                let ship: SCNNode = scene.rootNode.childNodes.first!
                ship.position = position2
                sceneView.scene.rootNode.addChildNode(ship)
    }
...
// MARK: - Регистрация столкновений (нужно подписаться в contactDelegate (viewDidLoad))
...
extension ViewController: SCNPhysicsContactDelegate {
    ...
    func physicsWorld(_ world: SCNPhysicsWorld, didBegin contact: SCNPhysicsContact) {
        ...
        // если битовая маска = box
        if nodeA.physicsBody?.contactTestBitMask == BitMaskCategory.box {
            // изменим цвет куба
            nodeB.geometry?.materials.first?.diffuse.contents = UIColor.green
            return
        } else {
            // проверяем другие и что то делают
        }
    }
}
